...
IRIS_ROOT = Path(__file__).resolve().parent
LIBRARY_ROOT = IRIS_ROOT / "double_secure_iris_library_registration"
...
class IrisLibraryRegistration:
    ...
    def register(
        ...
        self._write_pair(f"{iris_id}.json", payload)
        record = self.trust_registry.issue_record(
            record_type="iris-registration",
            source="IrisLibraryRegistration",
            payload=payload,
        )
        return {"library_entry": iris_id, "trust_record_hash": record.integrity_hash}

    def delete(self, iris_id: str) -> dict[str, Any]:
        removed = []
        for root in (CANONICAL_DIR, WORKING_DIR):
            path = root / f"{iris_id}.json"
            if path.exists():
                path.unlink()
                removed.append(str(path))
        record = self.trust_registry.issue_record(
            record_type="iris-deletion",
            source="IrisLibraryRegistration",
            payload={"iris_id": iris_id, "removed_paths": removed},
        )
        return {"iris_id": iris_id, "removed_paths": removed, "trust_record_hash": record.integrity_hash}
